...
<div class="row g-3">
    ...
    <div class="col-md-4"><label class="form-label fw-semibold">Child status</label><select class="form-select" name="student_status"><?php foreach (array('active', 'inactive') as $status): ?><option value="<?= e($status) ?>" <?= $studentStatus === $status ? 'selected' : '' ?>><?= e(ucfirst($status)) ?></option><?php endforeach; ?></select></div>
