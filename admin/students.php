...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'delete') {
            $pdo->beginTransaction();
            delete_student_tree((int) post_value('student_id'));
            $pdo->commit();
...
admin_header('Children', 'students', 'Manage child records, parent links, school information, and pickup details.');
?>

<div class="card border-0 shadow-sm">
    <div class="card-header bg-white d-flex flex-wrap align-items-center justify-content-between gap-2">
        <div><h2 class="h6 mb-0">Child Records</h2><span class="text-secondary small"><?= count($students) ?> total records</span></div>
        <button class="btn btn-primary btn-sm" type="button" data-bs-toggle="modal" data-bs-target="#createStudentModal" <?= !$parents ? 'disabled' : '' ?>><i class="bi bi-plus-lg me-1"></i>Add child</button>
    </div>
    <div class="card-body">
        <?php if (!$parents): ?>
            <div class="alert alert-warning">Create at least one parent before adding children.</div>
        <?php endif; ?>
        <div class="table-responsive">
            <table class="table align-middle mb-0">
                <thead><tr><th>Child</th><th>Parent</th><th>LRN</th><th>School</th><th>Grade</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
                <tbody>
                    <?php foreach ($students as $student): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                <form class="d-inline" method="post" data-confirm="Delete this child? Delete will fail if bookings still depend on this record.">
                    ...
                    <?php endforeach; ?>
                    <?php if (!$students): ?><tr><td colspan="7" class="text-center text-secondary py-4">No children yet.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>
...
<div class="modal fade" id="createStudentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable"><form class="modal-content" method="post">
        ...
        <div class="modal-header"><h3 class="modal-title h5">Add Child</h3><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
        <div class="modal-body"><?php include __DIR__ . '/partials/student_fields.php'; ?></div>
        <div class="modal-footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary" type="submit">Save child</button></div>
    </form></div>
</div>
...
<?php foreach ($students as $record): ?>
    <div class="modal fade" id="editStudent<?= (int) $record['id'] ?>" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable"><form class="modal-content" method="post">
            ...
            <div class="modal-header"><h3 class="modal-title h5">Edit Child</h3><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
            <div class="modal-body"><?php include __DIR__ . '/partials/student_fields.php'; ?></div>
            <div class="modal-footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary" type="submit">Update child</button></div>
        </form></div>
    </div>
<?php endforeach; ?>
